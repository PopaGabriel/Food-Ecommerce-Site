class ContainerAddedIngredients {
  constructor(base, options) {
    ...
    this.listResults = [];
    ["search", "search__ingredients-container"].forEach((elem) =>
      base.classList.add(elem)
    );
    [
      "search__ingredients-container",
      "search__ingredients-container--visible",
    ].forEach((elem) => this.elements.resultContainer.classList.add(elem));
    ...
    [this.elements.resultTitle, this.elements.resultContainer].forEach((elem) =>
  ...
  addResultElement(elem) {
    ...
    if (
      Array.from(this.elements.resultContainer.children).filter(
        (val) => val.innerText.toLowerCase() === elem[1].toLowerCase()
      ).length > 0
    )
      return;
